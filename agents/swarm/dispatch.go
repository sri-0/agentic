...
func dispatchRun(
	...
	return func(ctx agent.InvocationContext) iter.Seq2[*session.Event, error] {
		return func(yield func(*session.Event, error) bool) {
			...
			// Partition pending tasks: the output agent (writer) runs in a
			// LAST phase, after every other worker finishes, so the final answer
			// streams at the end (it also sees their results on the board).
			var normal, last []int
			for i, t := range tasks {
				if t.Status != "pending" {
					continue
			...
			// runBatch runs one parallel batch; returns true if the caller should
			...
			runBatch := func(batch []int) bool {
				...
				// Workers stream events through this channel so the single
				// dispatch goroutine (the only safe place to yield) forwards them
				// live, instead of buffering until the whole batch finishes.
				...
				for bi, taskIdx := range batch {
					...
					go func(bi, taskIdx int) {
						defer wg.Done()
						task := tasks[taskIdx]
						// Unique per-task identity so multiple tasks of the same
						// worker type are individually addressable in the UI.
				...
				for event := range events {
					if stopped {
						continue // keep draining so workers don't block on send
					}
					if !yield(event, nil) {
						stopped = true
					}
			...
			// Other workers first, then the output agent (writer) last.
			if runPhase(normal) {
				return
			}
			if runPhase(last) {
